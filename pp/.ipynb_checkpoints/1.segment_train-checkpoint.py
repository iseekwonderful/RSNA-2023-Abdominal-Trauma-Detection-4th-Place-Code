...
files = glob.glob(path + '/../input/2d_mask/*.npy')
...
class SEGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = imread(row.file.replace('npy', 'png').replace('2d_mask', 'pngs'))
...
torch.save(model.state_dict(), path + '/../input/4_classes_seg_model_v2.pth')

print('############################## Pretrain ###################################')

for model_name in ['pvt2_b2', 'pvt2_b3', 'pvt2_b4', 'efficientnet-b0', 'tu-maxvit_tiny_tf_384']:
    model = smp.Unet(
        encoder_name=model_name,        # choose encoder, e.g. mobilenet_v2 or efficientnet-b7
        encoder_weights="imagenet",     # use `imagenet` pre-trained weights for encoder initialization
        in_channels=3,                  # model input channels (1 for gray-scale images, 3 for RGB, etc.)
        classes=4,     
        decoder_channels=(128, 64, 32, 16, 8), # model output channels (number of classes in your dataset)
    ).cuda()
    ...
    cd = SEGDataset(file_df[file_df.fold == 1].sample(frac=1), tfms=transforms_train)
    train_dl = DataLoader(cd, num_workers=16, batch_size=16)

    val_ds = SEGDataset(file_df[file_df.fold != 1].sample(frac=1), tfms=transforms_valid)
    valid_dl = DataLoader(val_ds, num_workers=16, batch_size=16)
    ...
    for epoch in range(1):
        ...
        for image, targets in tqdm.tqdm(valid_dl):
            with torch.no_grad():
                ...
                ious.append(((r > 0.5) * targets).sum() / (((r > 0.5) + targets) > 0.5).sum())
        print(f'Model: {model_name}, Epochs: {epoch}, iou: {np.mean(ious)}')
        torch.save(model.state_dict(), path + f'/../input/4_classes_{model_name}_pretrained.pth')
