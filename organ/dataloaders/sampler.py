...
class DonorBatchSampler(Sampler):
    def __init__(self, df, batch_size):
        super().__init__(self)
        self.df = df.reset_index()
        self.batch_size = batch_size
        self.unique_donor = df.donor.unique()
        print('[W] Using Batch Batch sampler ^_^ we have donor length: {}'.format(df.donor_id.unique()))

    def __iter__(self):
        for x in range(len(self.df) // self.batch_size):
            donor = random.choice(self.unique_donor)
            selected = self.df[self.df.donor_id == donor].sample(self.batch_size)
            # pos = self.df[self.df.target == 1].sample(cnt).index
            # neg = self.df[self.df.target == 0].sample(self.batch_size - cnt).index
            # index = list(self.df[self.df.experiment == random.sample(self.targets, 1)[0]].sample(16).index)
            # print(pos)
