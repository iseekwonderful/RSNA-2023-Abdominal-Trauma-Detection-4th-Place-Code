...
def dice(mode='binary', smooth=0.0):
    dice_loss = segmentation_models_pytorch.losses.DiceLoss(mode=mode, smooth=smooth)
    def _dice(output, truth):
        return dice_loss(output, truth)
    return _dice


def dice_bce_mix(mode='binary', smooth=0.0, bce_weights=100):
    dice_loss = segmentation_models_pytorch.losses.DiceLoss(mode=mode, smooth=smooth)
    bce = BCEWithLogitsLoss()
    def _bce_bce_loss(output, truth):
        return bce_weights * bce(output, truth) + dice_loss(output, truth)
    return _bce_bce_loss
