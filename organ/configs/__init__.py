...
class Data(Element):
    def __init__(self, dict):
        ...
        self.csv = dict.get('csv', 'whale_naive_split.csv')
        self.diff = dict.get('diff', -1)
        self.filter_empty = dict.get('filter_empty', 0)
        self.N = dict.get('N', 16)
        self.organ = dict.get('organ', 'spleen')
        self.organ2index = {
            'spleen': 1,
            'liver': 3,
            'bowel': 2,
        ...
        self.orgin_index = self.organ2index[self.organ]
...
class Model(Element):
    def __init__(self, dict):
        ...
        if not self.from_checkpoint[0] == '/' and not self.from_checkpoint == 'none':
            # relative path
            self.from_checkpoint = Path(os.path.dirname(os.path.realpath(__file__)) + '/../../input/') / self.from_checkpoint
...
class Loss(Element):
    def __init__(self, dict):
        ...
        self.sub_weight = dict.get('sub_weight', [1, 1, 1, 1, 1, 1])
        self.ch_weight = dict.get('ch_weight', 1)
