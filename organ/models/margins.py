...
class ArcModule(nn.Module):
    ...
    def forward(self, inputs, labels):
        ...
        cos_th_m = torch.where(cos_th > self.th.half(), cos_th_m.half(), cos_th - self.mm.half())
...
class EfficinetNetArcFace(nn.Module):
    def __init__(self, name='efficientnet_b0', pretrained='imagenet', out_features=1724,
                 dropout=0.5, feature_dim=512, m=0.5):
        super().__init__()
...
class ConvNextCosine(nn.Module):
    def __init__(self, name='convnext_base_384_in22ft1k', pretrained='imagenet', out_features=1724,
                 dropout=0.5, feature_dim=512, m=0.1):
        super().__init__()
        ...
        self.feature_linear = nn.Linear(in_features=in_features, out_features=feature_dim)
        self.last_linear = nn.Linear(in_features=feature_dim, out_features=out_features)
        self.pool = GeM()
        self.margin = DistLinear(indim=feature_dim, outdim=out_features)
        self.dropout = dropout
...
class ConvNextCosine2(nn.Module):
    def __init__(self, name='convnext_base_384_in22ft1k', pretrained='imagenet', out_features=1724,
                 dropout=0.5, feature_dim=512, m=0.1):
        super().__init__()
        ...
        self.model.head = torch.nn.Identity()
        self.pool = GeM()
        self.margin = nn.Linear(in_features=in_features, out_features=out_features)
        self.dropout = dropout

    def forward(self, x, label):
        x = self.model(x)
        f = nn.Flatten()(self.pool(x))
        features = F.normalize(f)
        if label is None:
            return features
        else:
...
# class EfficinetNetCosine(nn.Module):
#     def __init__(self, name='efficientnet_b0', pretrained='imagenet', out_features=1184,
#                  dropout=0.5, feature_dim=512, m=0.1):
#         super().__init__()
#         self.model = torch.hub.load('rwightman/gen-efficientnet-pytorch', name,
#                                     pretrained=(pretrained == 'imagenet'))
...
#         self.pool = nn.AdaptiveAvgPool2d(1)
#         self.margin = DistLinear(indim=self.model.classifier.in_features, outdim=out_features)
#         self.dropout = dropout
#
#     def forward(self, x, label):
#         x = self.model.features(x)
