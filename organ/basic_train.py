    ...
    try:
        ...
        for epoch in range(cfg.train.num_epochs):
            ...
            for i, data in enumerate(tq):
                ...
                inputs, labels, mask = data[0], [e.cuda() for e in data[1:-1]], data[-1]
                bs, N, d, w, h = inputs.shape
                inputs = inputs.reshape(bs * N, d, w, h).cuda()
                mask = mask.reshape(bs * N, 4, w, h).cuda()
                ...
                if cfg.train.cutmix and cfg.train.beta > 0 and r < cfg.train.cutmix_prob:
                    ...
                    if cfg.basic.amp == 'Native':
                        ...
                        loss_cls = train_loss([o.float() for o in outputs[1:]], [l.long() for l in labels], cfg=cfg)
            ...
            if len(valid_dl) > 0:
                validate_loss, accuracy, auc, bce_loss, mse_loss, ap, f103, f105, pf1 = basic_validate(model,  valid_dl, loss_func, cfg, tune)
            else:
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(tqdm.tqdm(dl)):
            ...
            if cfg.basic.amp == 'Native':
                ...
                dices.append(
                    ((mask.cpu() * (out_mask.cpu() > 0.3)).sum() / ((mask.cpu() + (out_mask.cpu() > 0.3)) > 0.5).sum()).item())
                predicted.append(torch.cat([torch.softmax(e.float(), 1) for e in outputs], 1).cpu().numpy())

                loss_cls = train_loss([o.float() for o in o[1:]], [l.long() for l in labels], cfg=cfg)
        ...
        scores, sub = score(gt.reset_index(), pred_df.drop('any_injury', axis=1), 'file')
        print('EVAL, loss_score: {}, dice: {}'.format(scores, np.mean(dices)))

        return np.mean(losses), np.mean(dices), sub['bowel'], sub['extravasation'], sub['kidney'], sub['liver'], sub['spleen'], sub['any'], scores
